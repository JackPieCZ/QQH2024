#Finding best corelation with PER and Team_PTS
import pandas as pd 
...
eligible_players = seasons_per_player[seasons_per_player >= 4].index
eligible_correlation_df = correlation_df[correlation_df['Player'].isin(eligible_players)]

# Find the player with the highest average correlation in any of the key metrics
average_correlation = eligible_correlation_df.groupby('Player')[['Team_PTS_CORR', 'Enemy_PTS_CORR', 'Won_CORR']].mean()

# Find the player with the highest correlation (max across columns)
best_player = average_correlation.mean(axis=1).idxmax()  # Take the index of the max average correlation

# Get the best player’s data
best_player_data = merged_data[merged_data['Player'] == best_player]

# Print the best player’s ID
print(f"Best player ID based on PER correlation: {best_player}")
